...
.directive('massAutocomplete', ["$timeout", "$window", "$document", "$q", function ($timeout, $window, $document, $q) {
  ...
  return {
    ...
    template:
      ...
      '<div class="ac-container" ng-show="show_autocomplete && results.length > 0" style="">' +
        '<ul class="ac-menu">' +
          '<li ng-repeat="result in results | unique: \'label\'" ng-if="$index > 0" ' +
    ...
    controller: ["$scope", function ($scope) {
      ...
      function _position_autocomplete() {
        ...
        container.style.top = rect.top + rect.height + scrollTop - 62 + 'px';
